...
**Superseded by DEC-039 (2026-07-18):** the Mira/Sky/Nunito direction was never loved (Eric) and is replaced wholesale by the Muster design system in the V3 UI redesign (Phase 10). This entry is retained as history.
...
## DEC-036: `courses.section_label` as a first-class field (2026-07-18)
**Decision:** Add a nullable `section_label varchar(50)` column to `courses`. It formalizes the disambiguator operators were cramming into `title` — e.g. "Open Sailing July 1st **Boat 1**" — so two concurrent same-time offerings become distinct courses distinguished *structurally* (Open Sailing · Boat 1 / Boat 2), not by title string. Display precedence app-wide becomes `course_type name` + optional `section_label` chip + a *derived* schedule; `title` stays for genuinely custom names. The course search/filter must include `section_label`. No RLS change — the column inherits `courses`' existing admin-write/all-read policies.
**Why:** Extends DEC-004 ("the course is the offering") and interacts cleanly with DEC-027 (each Open Sailing night is already its own course). The date/schedule itself is **derived from `sessions`, never denormalized onto `courses`** — a formatter renders "weekday · time · date-range" and must degrade gracefully ("Varies") for makeups/one-offs. Surfaced in the V3 redesign; closes #124's title-hack pain (the recurrence generator is the write side).
**Revisit if:** sections ever need their own attributes (a real boat entity with capacity/name) — then `section_label` graduates to a `boats`/`sections` FK. Not now.

## DEC-037: Instructor attendance capture via SECURITY DEFINER RPC (2026-07-18)
**Decision:** Let an instructor record attendance for **their own** assigned sessions through a `SECURITY DEFINER` RPC (`save_attendance(...)`) authorized to **admin OR the assigned instructor** — gated on `course.instructor_id = auth.uid() OR session.instructor_id = auth.uid()` (the DEC-007 override path). **No broad `session_attendance` RLS UPDATE grant** to instructors.
**Why:** `session_attendance` drives makeup logic (DEC-005/006) and completion; instructors are SELECT-only today, and instructor writes were already deliberately routed through an RPC for notes (4.6/4.11). Same pattern, same reason: the write must not let an instructor touch sessions they don't teach, must preserve the status vocabulary (`expected/present/absent/excused/missed`), and must not clobber cross-course `makeup_session_id` links. pgTAP coverage mirroring 4.6/4.11 is mandatory. Implements #146; the only invariant-touching change in the redesign.
**Revisit if:** substitute-assignment rules change such that "assigned instructor" needs a broader definition than course-default-or-session-override.

## DEC-038: Audit/activity log as an append-only `events` table (2026-07-18)
**Decision:** Model the admin activity log (#144) as `events(id, occurred_at, actor_id uuid null, actor_kind ['user'|'system'|'stripe'], event_type, entity_type, entity_id, summary, metadata jsonb)`, indexed on `(occurred_at desc)`, `(entity_type, entity_id)`, `(event_type)`. RLS: admin **SELECT** only; **INSERT** via SECURITY DEFINER / service role; **no UPDATE/DELETE** — immutability enforced by the DB, following the DEC-035 `credit_ledger` shape exactly. Emit from **server actions + explicit webhook/cron emitters, NOT per-table DB triggers** — triggers can't attribute the actor (`auth.uid()` is absent under service-role/SECURITY DEFINER, the same gap DEC-019 flagged).
**Why:** Admins have no way to reconstruct "who did what, when." The table is easy; the write mechanism is the real decision and the instrumentation is the real cost. Build **table + RLS + read-only feed first**, then wire emitters per category (enroll/pay/assign/session/attendance/notify/account) incrementally — an 8+, effectively a small subsystem, split by emitter. Notification fan-outs make it chatty, so the feed paginates ("Load older"); retention is a later concern.
**Revisit if:** volume forces retention/rollup, or a compliance need requires tamper-evidence beyond append-only RLS (hash-chaining).
**Implementation notes (2026-07-27, task 10.8):** Three details settled during build. (1) **`event_type` carries no CHECK constraint and no lookup table** — the vocabulary is a TS union in `src/lib/events.ts`, which also drives the feed's filter chips. Nothing joins to the column, so a constraint would buy no integrity while forcing a migration per emitter; the feed renders an unrecognized type verbatim so a row written by a newer deploy still reads on an older one. (2) **`entity_id` deliberately has no FK** — deleting a course or enrollment must leave its audit trail, which is exactly when you need it. (3) **UPDATE/DELETE don't raise, they no-op** — with no policy for those commands RLS simply exposes zero rows, so the pgTAP coverage proves immutability by the row surviving unchanged rather than by expecting a 42501. Scope landed as **foundation + the 7 enrollment/waitlist emitters only**: payment was considered first and rejected because Stripe and `payments.status`/`refund_amount_cents` already hold that history, whereas `enrollments` stores no actor and no history at all. Remaining categories (pay/assign/session/attendance/notify/account) stay open on #144/#163.

## DEC-039: Visual identity — adopt the Muster design system (2026-07-18)
**Decision:** Replace the Mira/Sky/Nunito theme (D-018) with the **Muster design language**: IBM Plex Sans (UI) + IBM Plex Mono (all meaningful numerals), harbor slate-blue accent (`#2f5d86` light / `#6ba3d6` dark) split into `--accent` (expressive) vs `--accent-solid` + `--on-accent` (fill behind white text — darkened to `#316293` in dark for AA), the semantic chip set (ok/warn/bad/info, each tint-bg + line), `--faint` at AA contrast, ~12px radius, two shadow levels, **both themes first-class** (light primary). A system **`:focus-visible` token** is part of the baseline. SailBook's distinct identity is its **sail-mark logo**, not a bespoke palette — the system is deliberately shared with Muster (near-zero cross-users; if separation is ever wanted, the accent hue is the one knob). Full tokens in `docs/BRAND.md`; reference implementation is the redesign mockup (all roles + public + auth).
**Why:** Eric never liked the original theme (light or dark). The Muster look tested strongly across the whole screen-by-screen redesign. Sharing a well-tuned system costs nothing here and buys consistency. @architect + @ui-reviewer both reviewed the mockup; the a11y baseline this bakes in (focus token, dark-mode contrast, `--faint` fix) is backported to Muster as muster#469.
**Consequence:** Supersedes D-018. `globals.css`, `.claude/ui-context.md`, and `@ui-reviewer` are rewritten to the new tokens in Phase 10 task 10.1 — no screen is built against the old theme.
**Revisit if:** SailBook ever needs visible separation from Muster (branding for an external audience) — shift the accent hue and diverge the logo further; the token structure already supports it.

## DEC-040: Token architecture — hex Muster palette + shadcn aliases (2026-07-19)
**Decision:** Implement DEC-039 as a two-layer token system in `globals.css` (task 10.1, ratified by @architect). The **Muster palette is authored in hex as the single source of truth** (`--brand`, `--accent-solid`, `--on-accent`, `--bg`, `--surface`, `--rail`, `--sink`, `--ink`, `--muted-ink`, `--faint`, `--line`, `--line-soft`, the ok/warn/bad/info chip triads, five `--t-*` course hues, two shadows). Every **shadcn token is a thin alias** over it (`--primary: var(--accent-solid)`, `--muted: var(--rail)`, `--muted-foreground: var(--muted-ink)`, `--accent: var(--sink)`, `--border: var(--line)`, `--ring: var(--brand)`, etc.), so shipped shadcn components re-skin with no markup change; new Phase-10 components consume the Muster vars (exposed as `@theme inline` utilities: `bg-surface`, `text-faint`, `bg-ok-bg`, …) directly.
**Two renames vs. the mockup** resolve hard CSS-property collisions (the same property can't hold two meanings): the mockup's `--accent` (expressive blue) → **`--brand`**, its `--accent-ink` → **`--brand-ink`**, and its `--muted` (text) → **`--muted-ink`** (a.k.a. shadcn `--muted-foreground`). shadcn keeps `--accent` as the subtle hover fill (`--sink`) and `--muted` as a background (`--rail`). Aliases are restated in both `:root` and `.dark` so the `var()` expressions re-resolve per theme (a custom property declared only on `:root` computes once with light values and inherits frozen).
**Why:** aliasing avoids rewriting every shipped shadcn component (pure deadline risk, zero benefit); hex avoids OKLCH conversion drift from BRAND/mockup and keeps `color-mix(in oklab, …)` hover-darkens working. Theme stays next-themes `attribute="class"` (`.dark`) + `defaultTheme="system"` — adopting the mockup's `data-theme`/`prefers-color-scheme` would re-litigate DEC-020's per-device theming for no gain.
**Consequence:** `--faint` adopts the mockup's AA-passing values (`#6a7482` / `#8792a1`); `docs/BRAND.md`'s stale non-AA row was corrected. When 10.2+ translate mockup CSS, `var(--accent)`→`var(--brand)`, `var(--accent-ink)`→`var(--brand-ink)`, `var(--muted)`(text)→`var(--muted-ink)`; any bespoke `:root[data-theme="dark"]` selectors become `.dark` selectors.
**Revisit if:** we ever drop shadcn — then the alias layer is dead weight and components consume Muster vars directly.
