...
## DEC-036: Credit-at-checkout settlement record + overdraw enforcement (2026-07-02)
**Decision:** A fully-credit-covered checkout (#107) writes a `payments` row — `payment_method: 'credit'`, `amount_cents` = credit applied, `status: 'succeeded'`, no Stripe fields — same as any other confirmed enrollment. `credit_ledger` balance non-negativity is enforced in the database via `trg_credit_ledger_prevent_overdraw` (migration `20260702171443`): a `BEFORE INSERT` trigger that takes a per-student advisory lock (`pg_advisory_xact_lock`) and rejects any insert that would take the balance negative.
**Why:** DEC-025 already established `payments` as the settlement record for every confirmed enrollment regardless of rail (`stripe | cash | check | venmo | stripe_manual`) — "no payments row, since no Stripe transaction happened" (the original #107 reasoning) contradicted that precedent. Concretely: `processRefund` and `issueCredit` (`src/actions/enrollments.ts`) both require a `succeeded` payments row and error with none found — so a student who fully covers a course with credit could never have *that* enrollment refunded or credited later, breaking the #108 transfer-by-composition scenario (cancel-with-credit, re-enroll) on its second hop if the destination course also needed to be cancelled. Caught in an @architect review, not the original build.
On the overdraw trigger: the app has no cross-statement transaction boundary at the application layer (Server Actions via supabase-js can't hold one; Vercel gives no cross-instance app lock), so "balance never goes negative" under concurrent redemption (two checkouts on different courses reading the same balance before either commits) is only enforceable in the database.
**Reversal:** voiding an erroneous credit issuance or redemption has no UI — insert an explicit negative (or positive, for a redemption reversal) ledger row via SQL, same posture as other admin-only manual-data-fix scenarios in this app. Not automated; low volume expected.
**Revisit if:** Admin needs to see a student's full credit *history*, not just the current balance (today: SELECT is RLS-permitted, just not rendered anywhere) — cheap to add if it comes up.
