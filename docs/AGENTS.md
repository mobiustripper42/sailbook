...
**Purpose:** Reviews visual design quality against SailBook's design system (`.claude/ui-context.md`).
...
**Purpose:** Classifies diffs between SailBook's live workflow files and the seeds template repo. Backports structural improvements (push) or forward-ports template changes (pull); flags cross-family patterns.
