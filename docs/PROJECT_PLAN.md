...
| 6.25 | Public course catalog at `/courses` — replaces LTSC `/product-category/course/` | 5 | **Priority: medium.** Andy ask: "if we have time I'd like to replace this." Public catalog page lists every active course type as a tappable card; cards link to existing `/courses/[slug]` detail (those stay as-is). Scope: new `src/app/(public)/courses/page.tsx`; responsive grid (1/2/3 col); each card shows name + short_code badge + cert_body badge + prereq badge + truncated description + "From $X" pricing (MIN price across active courses) + "View details" CTA; SEO `generateMetadata`; empty state with "Get notified" link. Decisions: **no images in V1** — typographic card with the code badge as visual anchor (`course_types.image_url` is V3 follow-up); brand stays SailBook (replacing the catalog page only, not the LTSC site chrome); add `course_types.sort_order INTEGER NOT NULL DEFAULT 0` for deterministic display order. Files: new page, new migration, seed bumps, Playwright spec, add `/courses` to the accessibility.spec.ts public list. Out of scope: hero images, user-facing sort/filter, full LTSC nav (Charters/Rentals/etc.), waitlist for not-yet-scheduled types. Confirm with Andy: hide course types with no active courses, or show "Coming soon"? |
| 6.26 | Admin courses list — sortable columns + search + status filter (mirror users list) | 3 | **Priority: medium.** `/admin/courses` is currently a flat server-rendered table sorted by `created_at desc` only; gets unwieldy as the course count grows. Mirror the `UsersList` pattern from `src/components/admin/users-list.tsx`: extract a client component `CoursesList` that takes the server-fetched rows. Add: text search across course title + course_type name + instructor name; status filter pill row (All / Draft / Active / Completed / Cancelled); sortable column headers (course name, instructor, enrolled count, status, created_at). Same `toggleSort` + `compareCourses` shape as `compareUsers`. Sort header click cycles asc/desc with a visual indicator. Keep the existing capacity/price columns and `hidden sm:table-cell` / `hidden md:table-cell` responsive rules. Files: new `src/components/admin/courses-list.tsx`, refactor `src/app/(admin)/admin/courses/page.tsx` to pass data through, Playwright spec covering search + each filter + each sort. No migration. RLS unchanged. |
...
| 6.27 | Restore cancelled enrollment | 3 | **Priority: medium.** Once an enrollment is cancelled, the `enrollments_course_id_student_id_key` unique constraint blocks any new row for that student × course pair — admins have no way to bring a student back. Scope: `restoreEnrollment` server action that flips status back to `confirmed` (with capacity check), Restore button on cancelled rows in admin course detail, pgTAP RLS test (admin only), Playwright cancel→restore. Payment record stays as-is — if the original was refunded, admin handles re-collection separately (out of scope for this task; flag in DEC if it gets messy). |
...
| 6.30 | Mobile calendar / list view for students | 5 | **Priority: high.** Student calendar has Calendar + List views; the calendar grid is unusable on 375px so students fall back to the List, which is currently a giant wall of text — and this is the primary surface students see on their phones. Scope: redesign the student list view at 375px — group by day with sticky day headers, compact session cards (date pill + course code + time + location), tap-to-expand for details. Possibly: a "today" anchor button that scrolls to the current day. Keep desktop list unchanged. Verify the file owning this is `src/components/student/sessions-list.tsx` (or whatever the shared list component is). Files: refactor the list component, mobile-only Playwright spec. No data model changes. Decision still open: tap-to-expand vs always-expanded — defer until task starts and try both at 375px. |
| 6.29 | Admin course-types list — sortable + name-as-edit-link + row menu (mirror users / courses pattern) | 2 | **Priority: low** ("if we have time"). `/admin/course-types` is currently a flat server-rendered table. Mirror the `UsersList` / `CoursesList` pattern: extract a client component `CourseTypesList` with sortable columns. **No filter, no search** — the catalog is small. Course type **name links to edit** instead of a separate Edit button. Add a `•••` row-end menu (Edit / Activate-Deactivate / Manage Prerequisites). Files: new `src/components/admin/course-types-list.tsx`, refactor `src/app/(admin)/admin/course-types/page.tsx` to pass data through, Playwright spec covering each sort + the row menu actions. No migration. RLS unchanged. |
