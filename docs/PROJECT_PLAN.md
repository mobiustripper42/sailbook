...
| 6.24 | Course detail page mobile rewrite — `/admin/courses/[id]` is broken below 1024px | 5 | **Priority: medium.** Triggered by 6.13: the time picker improvements only land cleanly at lg+. Below 1024px the page has multiple problems: header (title + 4 action buttons in a `flex justify-between`) squishes the title; sessions table forces horizontal overflow despite `overflow-x-auto` wrapper; inline edit row's TimeSelect dropdowns are too cramped in a narrow table cell; cards visibly cut off at the md transition (~770px). Scope: redesign the full page for mobile-first, not retrofit. Likely involves: card-based session list (not a table) at <md, action menu instead of header buttons, reconsidering inline edit vs. drawer/modal pattern. |
