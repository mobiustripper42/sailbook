...
- [~] **SMS** (if A2P approved) — student preferences set to SMS, trigger any notification (e.g., admin enrolls them manually with notify=on), SMS arrives within 30 sec. *(Session 135: deferred to post-V2 — A2P friction blocking indefinitely. SMS path gated off via `SMS_ENABLED` env var; re-run this smoke when A2P clears and the env flips.)*
