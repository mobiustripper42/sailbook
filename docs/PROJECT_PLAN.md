...
| 6.20 | Admin + instructor calendar views — month grid + list fallback, mirroring the student calendar (5.10) | 5 | **Priority: high — must ship before V2 release.** **Implementation plan (session 115):** Student calendar is course-centric (one course → multiple dates); admin/instructor are session-centric (one session → one date) — create `src/components/shared/sessions-calendar.tsx` with simpler `{id,date,label,href,cancelled?}[]` shape; student `CoursesCalendar` stays untouched. Admin page: `app/(admin)/admin/calendar/page.tsx` fetches all active sessions; `components/admin/admin-calendar-view.tsx` (client) handles Course Type + Instructor filter selects + view switcher. Instructor page: `app/(instructor)/instructor/calendar/page.tsx` scoped to `courses.instructor_id=user.id OR sessions.instructor_id=user.id` (DEC-007). Add Calendar nav link to all 4 nav files (admin-nav, admin-mobile-nav-drawer, instructor-desktop-nav, instructor-mobile-nav-drawer). List fallback at <640px. 3 desktop Playwright tests per role. |
...
| 6.23 | Preserve `?next=` through registration — user arriving from public course page who creates an account instead of logging in loses the enrollment destination | 2 | **Priority: medium.** Current flow: `/courses/asa101` → Enroll → `/login?next=/student/courses/[id]` → user clicks "Create account" → `?next=` param is dropped → after registration they land on student dashboard, not the course. Fix: (1) login page "Create account" link passes `?next=` forward: `/register?next=...`; (2) registration success (or post-email-confirm redirect) uses the `?next=` param. Depends on 6.19. |
...
| 6.21 | Sidebar fixed to viewport height — desktop sidebar scrolls with main content instead of staying fixed | 2 | **Priority: high.** All three layouts (`admin`, `instructor`, `student`) use `flex min-h-screen` with a plain `aside` — no height constraint, no sticky positioning. Fix: add `sticky top-0 h-screen overflow-y-auto` to the aside in all three `layout.tsx` files. Mobile nav (hamburger + drawer) is unaffected — aside is already `hidden` below `md`. |
| 6.22 | Form field preservation on server action error — all fields clear when action returns an error, user must re-enter everything | 5 | **Priority: high.** When a server action returns an error (e.g. duplicate Short Code unique constraint), forms using `defaultValue` re-render empty. Auth forms were fixed in s110 with controlled inputs. Every other form is still broken: course-type create/edit, course create/edit, session edit, user edit, student profile, notification preferences, etc. Fix: convert `defaultValue` → controlled `value` + `onChange` across all affected forms, or pass submitted values back through `useActionState`. Audit all forms first — likely 10+ forms affected. |
