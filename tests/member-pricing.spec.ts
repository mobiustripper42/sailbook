...
test.describe('Member pricing', () => {
  ...
  test('admin can mark student as Simply Sailing member', async ({ page }) => {
    ...
    // Name cell is now a link to the edit page (since user-row-actions refactor)
    const row = page.getByRole('row').filter({ hasText: 'PW Student' }).first()
    await row.getByRole('link', { name: 'PW Student' }).click()
    await page.waitForURL(/\/admin\/students\/.*\/edit/, { timeout: 10000 })
    await page.getByLabel('Member Pricing').check()
    await page.getByRole('button', { name: 'Save Changes' }).click()
  ...
  test.afterAll(async ({ browser }) => {
    ...
    try {
      ...
      await row.getByRole('link', { name: 'PW Student' }).click()
      await page.waitForURL(/\/admin\/students\/.*\/edit/, { timeout: 10000 })
      await page.getByLabel('Member Pricing').uncheck()
      await page.getByRole('button', { name: 'Save Changes' }).click()
      await page.waitForURL('/admin/users', { timeout: 10000 })
    } finally {
      await ctx.close()
