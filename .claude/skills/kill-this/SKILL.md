...
Resolve base branch — always the active trunk (DEC-S022):
```
BASE=main
```
`main` is the active trunk. A `production` branch, if the project has one, is a downstream deploy pointer advanced by `/promote-production` — never a PR base.
